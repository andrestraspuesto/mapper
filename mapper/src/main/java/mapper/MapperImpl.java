...
import mapper.exceptions.MappingException;

/**
 * Clase que mapea un objeto de una clase en otro de otra
 * teniendo en cuenta que los campos en ambas clases deben 
 * llamarse igual, aunque el tipo difiera
 * 
 * @author atraspuesto
 *
 */
public final class MapperImpl implements MapperInterface {
	 ...
	 * @param dstClass
	 * @param src
	 * @param depth depth[0] = profundidad actual, depth[1] profundidad maxima
	 * @return
	 */
	private <R, P> R map(Class<R> dstClass, P src, int[] depth){
		...
		try{		
			...
			for(Field field: srcFields){
				//Recorro todos los campos origen y los mapeo si existen en el destino	
				field.setAccessible(true);
				String name = getFieldName(field);
				Field currField = fieldInfoMap.get(name);
				
				if (currField != null) {	
					//obtengo la profundidad desde la que se parte y a la que debe llegarse
	...
	/**
	 * Determina la profundidad actual y maxima
	 * para este campo
	 * depth[0] profundidad actual
	 * depth[1] profundidad maxima
	 * depth = null si no hay limite de profundidad
	 * @param field
	 * @return
	 */
	...
	/**
	 * Crea un mapa que relaciona el nombre de cada atributo con el nombre
	 * al que responde de cara al mapeo
	 * @param dstClass
	 * @return
	 */
	...
	/**
	 * Mapea la instancia srcValue asociada al campo srcField
	 *  y se le asigna a dstField 
	 * @param srcValue
	 ...
	 */
	private <P, R> void mapField(P srcValue, R resultInstance, Field srcField, Field dstField, int[] depth)
			throws IllegalAccessException, ParseException, IllegalArgumentException, ClassNotFoundException, NoSuchFieldException, SecurityException, InstantiationException, InvocationTargetException, NoSuchMethodException {
		...
		if(dstField.getGenericType().equals(srcField.getGenericType())){
			...
			//es necesario transformar el objeto origen en uno del tipo de destino
	...
	/**
	 * Se encarga de mapear la instancia origen a la clase de destino cuando son de 
	 * distinto tipo
	 * @param srcValue
	 * @param srcField
	 * @param dstField
	 ...
	 * @throws IllegalArgumentException 
	 */
	private <P> Result convertFieldToDifferentClass(P srcValue, Field srcField, Field dstField, int[] depth)
			throws ClassNotFoundException, IllegalAccessException,
			NoSuchFieldException, ParseException, IllegalArgumentException, SecurityException, InstantiationException, InvocationTargetException, NoSuchMethodException {
	...
	/**
	 * Devuelve un objeto result que contiene como valor un objeto
	 * del tipo requerido por dstField, puede ser null.
	 * result.succes = false si srcValue no puede ser transformado
	 * en un objeto de la clase requerida por dstField
	 * 
	 * @param dstField
	 * @param srcField
	 * @param srcValue
	...
	/**
	 * Se encarga de mapear una instancia de un objeto complejo a otra 
	 ...
	 * @throws IllegalAccessException
	 */
	private <P> Result convertComplexObject(P srcValue, Field srcField, Field dstField, int[] depth, boolean tooDeep)
			throws IllegalAccessException {
		Result result;
		if (tooDeep) {
	...
	/**
	 * Se encarga del mapeo de instancias de coleciones genericas  cuando las clases de los
	 * objetos contenidos en las clases origen y destino son distintas
	 * @param dstField
	 * @param srcField
	 * @param srcValue
	 * @param depth
	...
	private <P> Result mapCollection(Field dstField, Field srcField, P srcValue, int[] depth) throws ClassNotFoundException, IllegalArgumentException, IllegalAccessException, NoSuchFieldException, SecurityException, InstantiationException, InvocationTargetException, NoSuchMethodException {
		...
		//Determino la clase que deben tener las instancias almacenadas en la coleccion de destino
	 ...
	 * @throws ClassNotFoundException
	 */
	private  Class<?> getClassOfGeneric(Field field)
			throws ClassNotFoundException {
	...
	/**
	 * Devuelve un string resultado de aplicar el patron definido
	 * en MappingNumberFormat o si no esta anotado el resultado
	 * del metodo toString de srcValue
	 * @param dstField
	 * @param n
	 * @return
	 */
	...
	/**
	 * Devuelve la fecha como un string formateada segun el patron definido
	 * en la anotacion MappingDateFormat
	 * @param dstField
	 * @param d
	 * @return
	 */
	...
	private Number stringToNumber(Class<?> dstClass, String str) throws ParseException {
		Number i = null;
		if(str != null){
			//Obtengo un number a partir de la conversion del string atendiendo al locale
	...
	private final static class Result {
		...
		/**
		 * indica si se ha podido tenido
		 * exito en la conversion
		 */
		private boolean success;
		
		/**
		 * Objeto resultante de la conversion
		 * null es un valor correcto
		 */
